...
bool LinkedList :: palindrome()
{
    // IMPLEMENT FUNCTION 
    Node* savedHead = head;
    bool isPalindrome = palindrome(&head, head);
    head = savedHead;
    return isPalindrome;
}

bool LinkedList :: palindrome(Node** front, Node* end) {
    if (!end) {
        return true;
    }
    bool sublist = palindrome(front, end->next);
    if (!sublist) {
        return false;
    }
    bool matching = ((*front)->data == end->data);
