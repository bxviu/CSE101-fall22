...
void BinHeap :: heapify(int index) {
    if (index*2 > size || (index*2+1 <= size && A[index] >= A[index*2] && A[index] >= A[index*2+1])) {
        return;
    }
    int swapIndex = 0;
    if (index*2+1 <= size && A[index] < A[index*2+1] && A[index*2] < A[index*2+1]) {
        swapIndex = index*2+1;
    }
    else if (index*2 <= size && A[index] < A[index*2]) {
        swapIndex = index*2;
    }
    else {
        return;
    }
    int swapTemp = A[index];
    A[index] = A[swapIndex];
    A[swapIndex] = swapTemp;
    heapify(swapIndex);
}
